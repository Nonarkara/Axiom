(function bootAxiomVNext() {
  ...
  function renderNav(copy) {
    ...
    setText('navStageRecord', copy.hero.nav.stageRecord);
    ...
    setText('mobileStageRecord', copy.hero.nav.stageRecord);
  ...
  function renderStageRecord(copy) {
    const section = copy.stageRecord;
    if (!section) return;

    setText('stageRecordEyebrow', section.eyebrow);
    setText('stageRecordTitle', section.title);
    setText('stageRecordLede', section.lede);

    const mount = document.getElementById('stageRecordMount');
    if (!mount) return;

    const history = state.evidence?.contentHistory || [];

    const cards = section.protocols.map((protocol) => {
      const galleryClass = protocol.photos.length === 3
        ? 'protocol-gallery is-triptych'
        : 'protocol-gallery is-solo';

      const photos = protocol.photos.map((photo) => `
        <img class="protocol-photo" src="${escapeHtml(photo.src)}" alt="${escapeHtml(photo.alt || '')}" loading="lazy">
      `).join('');

      const metaRows = (protocol.meta || []).map((row) => `
        <div class="protocol-meta-row">
          <span class="protocol-meta-label">${escapeHtml(row.label)}</span>
          <span class="protocol-meta-value">${escapeHtml(row.value)}</span>
        </div>
      `).join('');

      let referenceLink = '';
      if (protocol.reference && protocol.reference.historyId != null) {
        const record = history.find((item) => Number(item.id) === Number(protocol.reference.historyId));
        const url = safeUrl(record?.url || '');
        if (url) {
          referenceLink = `<a class="protocol-reference" href="${escapeHtml(url)}" target="_blank" rel="noopener">${escapeHtml(protocol.reference.label)} →</a>`;
        }
      }

      return `
        <article class="protocol-card" data-reveal>
          <div class="protocol-card-head">
            <span class="protocol-code">${escapeHtml(protocol.code)}</span>
            <span class="protocol-city">${escapeHtml(protocol.city)}</span>
            <span class="protocol-date">${escapeHtml(protocol.date)}</span>
          </div>
          <h3 class="protocol-headline">${escapeHtml(protocol.headline)}</h3>
          <div class="${galleryClass}">${photos}</div>
          <span class="protocol-venue">${escapeHtml(protocol.venue)}</span>
          <div class="protocol-meta">${metaRows}</div>
          ${referenceLink}
        </article>
      `;
    }).join('');
  ...
  function renderPublicRecord(copy) {
    ...
    const fieldProofCards = copy.publicRecord.fieldProofCards.map((card) => {
      const record = history.find((item) => Number(item.id) === Number(card.historyId));
    ...
    return `
      <article class="field-proof-card module-card" data-reveal>
        ...
        <div class="field-proof-body">
          ...
          <h3 class="card-title">${escapeHtml(localizedHistoryField(record, 'title'))}</h3>
          <p class="card-meta">${escapeHtml(localizedHistoryField(record, 'summary'))}</p>
